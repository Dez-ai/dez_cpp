...
int* func()
{
	int* a = new int(10);//开辟出一个新整型，初始值为10的新地址，返回该变量的地址
	return a;
}
...
//返回静态变量引用
int& test02() {
	static int a = 20;//静态变量放在全局区，程序结束后释放
	return a;
}
...
/*int func2(int a, int b=10，int c)//不能这么写
{
	return a + b;
}
*/
//2. 如果函数声明有默认值，函数实现的时候就不能有默认参数
//声明和实现只能有一个有默认参数，防止重复定义，不知道哪个是对的
...
int main() {
	...
	cout << "ret = " << func(20, 20) << endl;//传了就用这个值，如果没传就用默认值
...
//3. 以值方式返回局部对象，像局部变量一样，调用完成后会被释放
Person doWork2()
{
	Person p1;
	cout << (int *)&p1 << endl;
...
public:
	...
	Person(const Person& p) {
		...
		m_age = p.m_age;
        //m_height= p.m_height;//编译器默认实现得这行代码就叫浅拷贝
...
private:
	static int m_B; //静态成员变量也是有访问权限的，私有权限访问不到
};
int Person::m_A = 10;
int Person::m_B = 10;
...
public:
	...
	void ShowPerson() {
		if (this == NULL) {
			return;
		}//不让代码走到下一行，防止报错，提高了代码的鲁棒性
		cout << mAge << endl;//用到了this指针，
	}
...
void test01()
{
	SheepTuo st;
	st.Sheep::m_Age = 100;
	st.Tuo::m_Age = 200;//就只有一个这个数据，相当于数据共享
...
//多态满足条件： 
//1、有继承关系
//2、子类重写父类中的虚函数.........重写的概念：函数返回值类型 函数名 参数列表 完全相同，子类在重写的时候virtual关键字可写可不写
//多态使用：
//父类指针或引用指向子类对象
